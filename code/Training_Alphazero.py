...
class DualLogger(object):
    """A dual logger that writes output both to the console stream and to a log file."""
...
def update_and_save_charts(log_path, charts_dir):
    """Automatically parse the log file and silently plot/update evolution charts in real time."""
    import re
    import matplotlib
    matplotlib.use('Agg')  # Activate a non-interactive backend to avoid blocking the main training loop with GUI windows
...
def execute_episode(game, nnet):
    """
    Execute a single self-play episode using the current best neural network.
    ...
    # IMPORTANT: Create a new MCTS instance for every episode to clear the search tree memory
    ...
    while True:
        ...
        if r != 0:
            # Assign value labels to the collected states based on the final outcome
...
if __name__ == "__main__":
    ...
    for i in range(1, args['num_iters'] + 1):
        ...
        try:
            ...
            print(">>> [Auto-Plot] Real-time charts updated in 'TRAINNING_CHARTS/evolution_history.png'")
